...
ax[1].scatter(TestDataframe['Keq/Kpm_teo'],abs(TestDataframe['Keq/Kpm_teo']-TestDataframe['Keq/Kpm_est']), 
                                        color = 'lightsalmon', label = 'Am8_c34 Test Data')
ax[1].axhline(y = abs(TestDataframe['Keq/Kpm_teo']-TestDataframe['Keq/Kpm_est']).mean(), color = 'orangered', linestyle = '--', label = 'Mean')
ax[1].set_xlabel(xlabel = 'Theoretical Perm. (-)', fontsize = 10)
ax[1].set_ylabel(ylabel = 'Absolute Error', fontsize = 10)
#ax[1].set_xlim([1.00, 1.20])
#ax[1].set_ylim([-0.50, 100.00])
ax[1].xaxis.set_major_formatter(ticker.FormatStrFormatter('%.0f'))
ax[1].yaxis.set_major_formatter(ticker.FormatStrFormatter('%.0f'))
ax[1].legend(loc = "best", fontsize = 8)

fig.tight_layout()
fig.savefig('./vCNN/Topologies/'+modelName+'_ErrorAbs.png')
print('\nEnd\n')